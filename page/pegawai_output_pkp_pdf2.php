...
include ('../nigol.php');
	...
	if($row = mysqli_fetch_array($hasil2))*/
		...
		if($rowza = mysqli_fetch_array($hasil_lagi))
	...
	if (!$hasil2)
		die("Gagal Query data karena : ".mysqli_error());
	if($row = mysqli_fetch_array($hasil2))*/
	    $hasilnya  = mysqli_query("select * from tbl_pkp where tahun_pkp=$year AND dinilai='".$nip."' ORDER BY tahun_pkp");	
	...
	//======RIGHT=====
		...
		$html .="		<br>
					...
					$rowz[nama_jabatan]	<br>
						<br><br>
		<br><br><br><br><br><br>";
		while($row = mysqli_fetch_array($hasilnya)){
